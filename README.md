...
o	Download file from SharePoint to temporary file location (to a location under the users “temp” directory).
o	Upload to blob storage with same filename as was in SharePoint.
o	Delete temporary file on local filesystem.
o	Assuming no error, add a “migration success” registration for the file.
...
$cert = New-PnPAzureCertificate -OutPfx "AzureAutomationSPOAccess.pfx" - `
 ...
 -Country "ES" `
 -State "Madrid"
a.	This creates a “pfx” file in the location you run the command. This file we’ll upload to an Azure Key Vault later.

5.	Copy certificate public-key into Azure AD application manifest.
a.	Get credentials from the certificate object by outputting the “$cert.KeyCredentials” object from above command.
“$cert.KeyCredentials”
Output:
{
...
a.	Set the Application ID URI. By default this is “api://” + client ID, which is fine.
b.	Add a scope – named “access” for example. 
i.	Description: “Access API”
ii.	Admins and users can access.
iii.	Set as enabled.
c.	Copy scope URL & save for later. 
i.	Example: api://4d6522b1-ac89-4c02-a4ca-39181470d12c/access
9.	Save application details for configuration later:
a.	Client ID
b.	Tenant ID
c.	Secret value
d.	Certificate display name (“AzureAutomationSPOAccess”)
Setup Azure Resources
We need certain services created in Azure.
1.	Create a new resource group in the same region as your SharePoint Online tenant. “SPOColdStorage” for example.
a.	We’ll assume all resources go in the same region.
2.	Create a storage account, “cool” access tier; LRS (recommended). 
a.	Note down connection string.
b.	Enable CORs on storage account - https://docs.microsoft.com/en-us/azure/storage/blobs/quickstart-blobs-javascript-browser#setting-up
i.	For “blob service” allow “GET” and “OPTIONS” from “*”, or more recommended is from your website DNS (“https://spocoldstorage.azurewebsites.net”). 
ii.	Allowed headers: *
iii.	Exposed headers: *
iv.	Max age: 86400 (1 day)
 

3.	Create a “Key vault”. Note down vault URL (“https://spocoldstorage.vault.azure.net/”). 
a.	In key vault access policies, grant “get” permissions to secrets for the application ID created above. 
b.	Import the “pfx” file created above to the certificates store of the vault. Be sure to copy the same name & password as was used to create the certificate with.
4.	Create a “Service Bus Namespace” + queue with name “filediscovery”. 
a.	Basic tier should be fine.
b.	The queue max size should be 5gb (or the highest permitted for the performance tier).
c.	In the queue, add a shared access policy with permissions “read” and “send”.
...
a.	Recommended: use a DTU-based service tier – standard (S0) should be fine. Also “LRS” backup should be fine for this workload.
b.	The server needs SQL Server authentication enabled.
c.	Build a connection-string in a text editor like this:
...
o	This is so we don’t have to renew messages often for large file migrations.
 
•	Max delivery count: 1000 (default 10).
o	This gives a huge potential to parallel process files.
...
o	For urgent migrations it’s probably worth deploying the roles in a virtual-machine or VM set with high-performance disks and at least x8 cores each.
o	App services are great for a managed environment but don’t offer the best performance tier if you need raw power to shift huge datasets. 
...
o	The system will back-off with increasing delays if throttled; the indexer especially will parse the “retry after” response header to avoid being blocked.
o	Read more about SPO throttling here - https://docs.microsoft.com/en-us/sharepoint/dev/general-development/how-to-avoid-getting-throttled-or-blocked-in-sharepoint-online 
